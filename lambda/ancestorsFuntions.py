...
with open("./documents/family_tree.json") as infoAncest_doc:
    ANCESTORS = json.load(infoAncest_doc)
#with open("./documents/birthday.json") as birthday_doc:
#    CELEBS = json.load(birthday_doc)


def get_ancestor(idAncestor):
    
    
    if(idAncestor>=0 and idAncestor<len(ANCESTORS)):
        return ANCESTORS[idAncestor]
    else:
        return{
            "id":0, 
            "nombre":None, 
